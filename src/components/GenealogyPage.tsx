...
const GenealogyPage = () => {
  ...
  const [connectedPersonsData, setConnectedPersonsData] = useState<any>(null);
  const [extendedConnectedData, setExtendedConnectedData] = useState<any>(null);
  const [isLoadingConnected, setIsLoadingConnected] = useState(false);
  const [isLoadingExtended, setIsLoadingExtended] = useState(false);

  // Relatives Modal State
  const [showRelativesModal, setShowRelativesModal] = useState(false);
  const [relativesModalData, setRelativesModalData] = useState<any>(null);
  const [isFetchingRelatives, setIsFetchingRelatives] = useState(false);

  const handleRelativeCardClick = (mobileNumber?: string) => {
    if (!mobileNumber) return;

    skipFromSearchRef.current = true;
    setFromPhone(mobileNumber);
    setFromSuggestions([]);
  };

  const openRelativeModal = async (personId: number) => {
    if (!personId) return;
    console.log("📡 FETCHING RELATIVES FOR MODAL - ID:", personId);

    setIsFetchingRelatives(true);
    setRelativesModalData(null);
  ...
  const fetchConnectedPersons = useCallback(async (personId: number) => {
    if (!personId) return;
    console.log("📡 FETCHING CONNECTED PERSONS FOR ID:", personId);

    // Reset data
    setConnectedPersonsData(null);
    setExtendedConnectedData(null);

    // Fetch Depth 1
    setIsLoadingConnected(true);
    const fetchDepth1 = async () => {
      try {
        const data = await genealogyService.getConnectedPersons(personId, 1);
        console.log("📥 DEPTH 1 DATA RECEIVED:", data);
        setConnectedPersonsData(data);
      } catch (error) {
        console.error("🔴 Error fetching depth 1 connected persons:", error);
      } finally {
    ...
    const fetchDepth3 = async () => {
      try {
        const data = await genealogyService.getConnectedPersons(personId, 3);
        console.log("📥 DEPTH 3 DATA RECEIVED:", data);
        setExtendedConnectedData(data);
      } catch (error) {
        console.error("🔴 Error fetching depth 3 connected persons:", error);
      } finally {
  ...
  // Fetch connected persons for currentPerson on load or change
  useEffect(() => {
    if (currentPerson?.id) {
      console.log("🔄 currentPerson changed, fetching connections for:", currentPerson.id);
      fetchConnectedPersons(currentPerson.id);
    }
  }, [currentPerson?.id, fetchConnectedPersons]);
  ...
  useEffect(() => {
    ...
    // Initialize two-way mode from navigation state
    if (location.state?.mode === 'two-way') {
  ...
  // Fetch suggestions for Two-Way mode
  useEffect(() => {
    if (!isTwoWayMode) return;

    const fetchSuggestions = async (phone: string, setSugg: (s: any[]) => void, skipRef: React.MutableRefObject<boolean>) => {
      if (skipRef.current) {
      ...
      if (cleaned.length >= 3) {
        try {
          console.log(`🔍 Fetching suggestions for ${cleaned}...`);
          const response = await genealogyService.searchPersons(cleaned);
          const suggestions = response?.data?.results || response?.results || response?.suggestions || [];
          console.log(`📊 Suggestions found for ${cleaned}:`, suggestions.length);
          setSugg(suggestions);
        } catch (error) {
          console.error("Error fetching two-way suggestions:", error);
          setSugg([]);
        }
      } else {
        setSugg([]);
    ...
    const fromTimer = setTimeout(() => fetchSuggestions(fromPhone, setFromSuggestions, skipFromSearchRef), 500);
    const toTimer = setTimeout(() => fetchSuggestions(toPhone, setToSuggestions, skipToSearchRef), 500);

    return () => {
      clearTimeout(fromTimer);
      clearTimeout(toTimer);
    };
  }, [fromPhone, toPhone, isTwoWayMode]);
  ...
  // Two-way relation handlers
  const getPersonIdFromPhone = async (phone: string): Promise<number | null> => {
    console.log(`🔍 Resolving phone to ID: ${phone}`);
    try {
      const response = await genealogyService.searchPersons(phone);
      console.log(`📥 Search response for ${phone}:`, response);
      // Handle the nested structure from searchPersons response, including suggestions
      const resultsArray = response?.data?.results || response?.results || response?.data?.suggestions || response?.suggestions || [];
      console.log(`📊 Found ${resultsArray.length} potential matches/suggestions for ${phone}`);

      if (resultsArray.length > 0) {
        // Find exact match if possible, otherwise first result
        const match = resultsArray.find((r: any) =>
          (String(r.mobile_number || r.value || '').includes(phone)) ||
          (String(r.full_name || r.label || '').includes(phone))
        );
        const resolvedId = match ? match.id : resultsArray[0].id;
        console.log(`✅ Resolved ${phone} to ID: ${resolvedId}`);
        return resolvedId;
      }
      console.warn(`❌ No results found for phone: ${phone}`);
      return null;
    } catch (error) {
  ...
  const handleTwoWayConnect = async () => {
    console.log("🚀 handleTwoWayConnect CLICKED", { fromPhone, toPhone });

    if (!fromPhone || !toPhone) {
      console.warn("⚠️ Missing phone numbers");
      toast.error(isTamil ? 'இரண்டு எண்களையும் உள்ளிடவும்' : 'Please enter both numbers');
      return;
    }

    if (fromPhone.length < 10 || toPhone.length < 10) {
      console.warn("⚠️ Invalid phone length", { fromLen: fromPhone.length, toLen: toPhone.length });
      toast.error(isTamil ? 'சரியான 10 இலக்க எண்ணை உள்ளிடவும்' : 'Please enter valid 10-digit numbers');
      return;
    }
    ...
    try {
      console.log("🔄 Resolving From Phone...");
      const person1Id = await getPersonIdFromPhone(fromPhone);
      if (!person1Id) {
        console.warn(`❌ Person 1 (${fromPhone}) not found`);
        toast.error(`${isTamil ? 'எண்' : 'Number'} ${fromPhone} ${isTamil ? 'கண்டறியப்படவில்லை' : 'not found'}`);
        setIsConnecting(false);
        return;
      }

      console.log("🔄 Resolving To Phone...");
      const person2Id = await getPersonIdFromPhone(toPhone);
      if (!person2Id) {
        console.warn(`❌ Person 2 (${toPhone}) not found`);
        toast.error(`${isTamil ? 'எண்' : 'Number'} ${toPhone} ${isTamil ? 'கண்டறியப்படவில்லை' : 'not found'}`);
        setIsConnecting(false);
        return;
      }

      // Ensure we have a valid centerPersonId (number)
      const centerPersonId = Number(nodes['root']?.personId || currentPerson?.id || 1);
      console.log(`📡 Calling getRelationBetween with p1=${person1Id}, p2=${person2Id}, center=${centerPersonId}`);

      // Use full URL to be absolutely sure
      const response = await genealogyService.getRelationBetween(person1Id, person2Id, centerPersonId);
      console.log("📥 Relation Between Response RECEIVED:", response);

      if (response && response.success) {
        setTwoWayResult(response);
        toast.success(isTamil ? 'உறவுமுறை கண்டறியப்பட்டது' : 'Relationship path found');
        setShowSidePanel(true);
      } else {
        console.warn("⚠️ API reported failure or no path", response);
        toast.error(response?.message || (isTamil ? 'உறவுமுறை கண்டறிய முடியவில்லை' : 'Relationship path not found'));
      }
    } catch (error: any) {
      console.error('🔴 Error in two-way connect:', error);
      toast.error(error.message || (isTamil ? 'பிழை ஏற்பட்டது' : 'An error occurred'));
    } finally {
      setIsConnecting(false);
      console.log("🏁 handleTwoWayConnect FINISHED");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {/* Search Bar / Two-Way Relation UI */}
          <div className="relative flex flex-col items-start px-4">
            {isTwoWayMode ? (
              <div className="flex flex-col sm:flex-row gap-3 w-full max-w-2xl animate-fadeIn relative z-50">
                {/* From Field */}
                <div className="relative flex-1">
                  <Phone className="absolute left-3 top-1/2 transform -translate-y-1/2 text-amber-600/50" size={18} />
                  <input
                    type="tel"
                    value={fromPhone}
                    onChange={(e) => {
                      ...
                      setFromPhone(val);
                      if (val.length === 10) setFromSuggestions([]);
                    }}
                    placeholder={isTamil ? 'யாரிடமிருந்து (தொலைபேசி)...' : 'From (phone)...'}
                    className="w-full bg-white border border-amber-200 rounded-xl py-2.5 pl-10 pr-4 text-amber-900 placeholder-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-xl transition-all font-semibold"
                    maxLength={10}
                  />
                  {fromSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-amber-200 rounded-2xl shadow-2xl z-50 overflow-hidden animate-fadeIn backdrop-blur-md bg-white/95">
                      <div className="p-2 bg-amber-50 border-b border-amber-100 text-[9px] font-black text-amber-600 uppercase tracking-widest">{isTamil ? 'தேடல் முடிவுகள்' : 'Search Results'}</div>
                      <div className="max-h-60 overflow-y-auto">
                        {fromSuggestions.map((s) => (
                          <div
                            ...
                            onClick={() => {
                              skipFromSearchRef.current = true;
                              setFromPhone(s.mobile_number || s.value || '');
                              setFromSuggestions([]);
                              // Clear toSuggestions as well when clicking from suggestion
                              setToSuggestions([]);
                            }}
                            className="p-3 hover:bg-amber-100 cursor-pointer border-b border-amber-50 last:border-0 transition-all flex items-center gap-3 group"
                          >
                            <div className="w-8 h-8 rounded-full bg-amber-200 flex items-center justify-center text-amber-700 font-bold text-xs group-hover:bg-amber-600 group-hover:text-white transition-colors">
                              {(s.full_name || s.label || '?').charAt(0).toUpperCase()}
                            </div>
                            <div className="flex-1">
                              <div className="text-xs font-black text-amber-900">{s.full_name || s.label}</div>
                              <div className="text-[10px] text-amber-600 font-bold">{s.mobile_number || s.value}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const val = e.target.value.replace(/\D/g, '').slice(0, 10);
                      setToPhone(val);
                      if (val.length === 10) setToSuggestions([]);
                    }}
                    placeholder={isTamil ? 'யாருக்கு (தொலைபேசி)...' : 'To (phone)...'}
                    className="w-full bg-white border border-amber-200 rounded-xl py-2.5 pl-10 pr-4 text-amber-900 placeholder-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-xl transition-all font-semibold"
                    maxLength={10}
                  />
                  {toSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-amber-200 rounded-2xl shadow-2xl z-50 overflow-hidden animate-fadeIn backdrop-blur-md bg-white">
                      <div className="p-2 bg-amber-50 border-b border-amber-100 text-[9px] font-black text-amber-600 uppercase tracking-widest">{isTamil ? 'தேடல் முடிவுகள்' : 'Search Results'}</div>
                      <div className="max-h-60 overflow-y-auto">
                        {toSuggestions.map((s) => (
                          <div
                            ...
                            onClick={() => {
                              ...
                              setToPhone(s.mobile_number || s.value || '');
                              setToSuggestions([]);
                              // Clear fromSuggestions as well when clicking to suggestion
                              setFromSuggestions([]);
                            }}
                            className="p-3 hover:bg-amber-100 cursor-pointer border-b border-amber-50 last:border-0 transition-all flex items-center gap-3 group"
                          >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={handleTwoWayConnect}
                    disabled={isConnecting}
                    className={`px-6 py-2.5 rounded-xl bg-linear-to-r from-amber-600 to-amber-700 text-white font-bold shadow-lg hover:from-amber-700 hover:to-amber-800 transition-all flex items-center justify-center gap-2 min-w-[120px] ${isConnecting ? 'opacity-70 cursor-not-allowed' : 'active:scale-95'}`}
                  >
                    {isConnecting ? (
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl bg-white border border-amber-200 text-amber-700 hover:bg-amber-50 transition-all shadow-md active:scale-95"
                    title={isTamil ? 'வெளியேறு' : 'Exit'}
                  >
                    <X size={20} />
                  </button>
                </div>

                {/* Connected Persons Dual Display (Top Bar Area - Compact Left-Aligned) */}
                {(connectedPersonsData || extendedConnectedData || isLoadingConnected || isLoadingExtended) && (
                  <motion.div
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="absolute top-full -left-2 mt-20 w-80 sm:w-96 bg-white/95 border border-amber-200 rounded-2xl shadow-2xl z-40 overflow-hidden backdrop-blur-md overflow-y-auto max-h-[80vh]"
                    onMouseDown={e => e.stopPropagation()}
                  >
                    <div className="flex flex-col divide-y divide-amber-100">

                      {/* Depth 1 Card (Close Connections) */}
                      <div className="flex flex-col min-w-0">
                        <div className="p-3 bg-amber-50/50 border-b border-amber-100 flex items-center justify-between sticky top-0 z-10 backdrop-blur-sm">
                          <div className="flex items-center gap-2">
                            <div className="p-1.5 bg-amber-100 rounded-lg">
                              <Users size={14} className="text-amber-600" />
                            </div>
                            <span className="text-[10px] font-black text-amber-700 uppercase tracking-widest">
                              {isTamil ? 'நெருங்கிய உறவுகள் (Depth 1)' : 'Close Connections (Depth 1)'}
                            </span>
                          </div>
                          {connectedPersonsData && (
                            <div className="bg-amber-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-sm">
                              {connectedPersonsData.connected_count || connectedPersonsData.connected_persons?.length || 0}
                            </div>
                          )}
                        </div>

                        <div className="p-2 space-y-2 max-h-100 overflow-y-auto custom-scrollbar">
                          {isLoadingConnected ? (
                            ...
                            <>
                              {connectedPersonsData?.connected_persons?.map((item: any) => {
                                const person = item.person || item;
                                const label = item.relation_label?.label || item.relation_to_center;
                                return (
                                  <div
                                    key={person.id}
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-baseline justify-between gap-2">
                                        <div className="text-[11px] font-black text-amber-950 uppercase truncate group-hover:text-amber-600 transition-colors">
                                          {person.full_name}
                                        </div>
                                        <div className="text-[9px] font-bold text-amber-500 shrink-0">
                                          {label}
                                        </div>
                                      </div>
                                      {person.mobile_number && (
                                        <div className="text-[9px] text-amber-600/70 font-bold mt-0.5">
                                          {person.mobile_number}
                                        </div>
                                      )}
                                      <div className="text-[9px] text-gray-500 font-medium truncate italic mt-0.5 flex items-center gap-1 opacity-70">
                                        {item.relationship_path?.split(' -> ').join(' → ')}
                                      </div>
                                    </div>
                                    ...
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        openRelativeModal(person.id);
                                      }}
                                      className="bg-amber-100 hover:bg-amber-200 text-amber-800 px-2 py-1 flex items-center gap-1 rounded-md text-[10px] shadow-sm transition-all z-10 shrink-0 uppercase tracking-widest font-black"
                                    >
                                      {isTamil ? 'பார்' : 'View'}
                                    </button>

                                    <ChevronRight size={12} className="text-amber-200 group-hover:text-amber-500 transition-all group-hover:translate-x-0.5 shrink-0" />
                                  </div>
                                );
                              })}
                              {(!connectedPersonsData?.connected_persons || connectedPersonsData.connected_persons.length === 0) && (
                                <div className="text-center py-10 opacity-40">
                                  <Users size={24} className="mx-auto mb-2 text-amber-200" />
                                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
                                    {isTamil ? 'உறவுகள் இல்லை' : 'No connections'}
                                  </p>
                                </div>
                              )}
                            </>
                      ...
                      {/* Depth 3 Card (Extended Connections) */}
                      <div className="flex flex-col min-w-0 bg-amber-50/20">
                        <div className="p-3 bg-orange-50/50 border-b border-orange-100 flex items-center justify-between sticky top-0 z-10 backdrop-blur-sm">
                          <div className="flex items-center gap-2">
                            <div className="p-1.5 bg-orange-100 rounded-lg">
                              <Link size={14} className="text-orange-600" />
                            </div>
                            <span className="text-[10px] font-black text-orange-700 uppercase tracking-widest">
                              {isTamil ? 'விரிவான உறவுகள் (Depth 3)' : 'Extended Connections (Depth 3)'}
                            </span>
                          </div>
                          {extendedConnectedData && (
                            <div className="bg-orange-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-sm">
                              {extendedConnectedData.connected_count || extendedConnectedData.connected_persons?.length || 0}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="p-2 space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar">
                          {isLoadingExtended ? (
                            <div className="flex flex-col items-center justify-center py-10 gap-3">
                              ...
                              <span className="text-[9px] font-black text-orange-500 uppercase tracking-widest animate-pulse italic">
                                {isTamil ? 'தேடுகிறது...' : 'Searching...'}
                              </span>
                            </div>
                          ) : (
                            <>
                              {extendedConnectedData?.connected_persons?.map((item: any) => {
                                const person = item.person || item;
                                const label = item.relation_label?.label || item.relation_to_center;
                                return (
                                  <div
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-baseline justify-between gap-2">
                                        <div className="text-[11px] font-black text-orange-950 uppercase truncate group-hover:text-orange-600 transition-colors">
                                          {person.full_name}
                                        </div>
                                        <div className="text-[9px] font-bold text-orange-500 shrink-0">
                                          {label}
                                        </div>
                                      </div>
                                      {person.mobile_number && (
                                        <div className="text-[9px] text-orange-600/70 font-bold mt-0.5">
                                          {person.mobile_number}
                                        </div>
                                      )}
                                      <div className="text-[9px] text-gray-500 font-medium truncate italic mt-0.5 flex items-center gap-1 opacity-70">
                                    ...
                                    <button
                                      ...
                                      className="bg-orange-100 hover:bg-orange-200 text-orange-800 px-2 py-1 flex items-center gap-1 rounded-md text-[10px] shadow-sm transition-all z-10 shrink-0 uppercase tracking-widest font-black"
                                    >
                                      {isTamil ? 'பார்' : 'View'}
                                    </button>

                                    <ChevronRight size={12} className="text-orange-200 group-hover:text-orange-500 transition-all group-hover:translate-x-0.5 shrink-0" />
                                  </div>
                                );
                              })}
                              {(!extendedConnectedData?.connected_persons || extendedConnectedData.connected_persons.length === 0) && (
                                <div className="text-center py-10 opacity-40">
                                  <Link size={24} className="mx-auto mb-2 text-orange-200" />
                                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
                                    {isTamil ? 'உறவுகள் இல்லை' : 'No connections'}
                                  </p>
                                </div>
                    ...
                    {/* Compact Footer */}
                    <div className="p-2 bg-gray-50 border-t border-amber-100 flex justify-end">
                      <button
                        onClick={() => { setConnectedPersonsData(null); setExtendedConnectedData(null); }}
                        className="text-[9px] font-black text-gray-400 hover:text-amber-600 uppercase tracking-widest transition-colors flex items-center gap-1"
                      >
                        <X size={10} />
                        {isTamil ? 'மூடு' : 'Close'}
                      </button>
        ...
        >
          ...
          {!isPanelCollapsed && (
            <div
              ...
              className="flex-1 overflow-auto p-4 bg-white custom-scrollbar"
            >
              {/* Two-Way Relation Result */}
              {isTwoWayMode && twoWayResult && (

                <div className="mb-12 animate-fadeIn">

                  {/* Vertical Journey View */}
                  <div className="relative px-2 pt-4 min-w-[400px]">
                    {/* Central vertical line */}
                    <div className="absolute left-1/2 top-4 bottom-4 transform -translate-x-1/2 w-0.5 bg-linear-to-b from-amber-100 via-amber-400 to-amber-100"></div>

                    <div className="space-y-0 relative flex flex-col items-center">
                      {/* Processed Relations for Journey */}
                      {(() => {
                        const filteredRelations = twoWayResult.cumulative_relations?.filter((rel: any, i: number, arr: any[]) => {
                          if (i === arr.length - 1 && arr.length > 0) {
                            const personName = rel.person?.name?.toLowerCase() || '';
                            const label = rel.cumulative_label?.toLowerCase() || '';
                            const firstPersonName = twoWayResult.person1?.name?.toLowerCase() || '';
                            if (personName === firstPersonName || personName.includes('me') || personName.includes('self') || label.includes('self')) {
                              return false;
                            }
                          }
                          return true;
                        ...
                        return (
                          <>
                            ...
                            <div className="relative group mb-0">
                              <div className="w-22 h-22 rounded-full bg-linear-to-br from-amber-500 to-amber-600 border-2 border-amber-400 shadow-xl flex flex-col items-center justify-center p-2 z-10 transition-all duration-300 group-hover:scale-110 group-hover:shadow-amber-200">
                                <User size={24} className="text-white mb-0.5" />
                                <div className="text-[9px] font-black text-white text-center leading-tight truncate w-16 uppercase">
                                  {twoWayResult.person1?.name}
                                </div>
                              </div>
                              <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-20">
                                <div className="bg-amber-100 text-amber-700 text-[8px] font-black px-2 py-0.5 rounded-full border border-amber-200 shadow-sm uppercase tracking-widest">FROM</div>
                              </div>
                            </div>

                            {/* Journey Steps */}
                            {filteredRelations.map((rel: any, idx: number) => {
                              return (
                                <React.Fragment key={idx}>
                                  {/* Connection Line with Stacked Labels (CENTER) */}
                                  <div className="flex flex-col items-center my-4 relative mx-auto z-20">
                                    <div className="w-1 h-8 bg-linear-to-b from-amber-200 to-amber-300 rounded-full"></div>

                                    <div className="relative my-2 z-10">
                                      {(() => {
                                        const fullLabel = rel.step_arrow_label || '';
                                        const labelParts = fullLabel.split(/->|→| - /);
                                        const hasSplit = labelParts.length > 1;

                                        // Strip underscores from backend to match frontend styling
                                        const topLabel = hasSplit ? labelParts[0].replace(/_/g, ' ').trim() : '';
                                        const bottomLabel = hasSplit ? labelParts[1].replace(/_/g, ' ').trim() : fullLabel.replace(/_/g, ' ').trim();

                                        return (
                                          <>
                                            {/* Label ABOVE arrow */}
                                            {hasSplit && topLabel && (
                                              <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 whitespace-nowrap z-30 mb-2">
                                                <span className="text-[9px] font-black text-amber-100 bg-amber-600 border border-amber-500 shadow-sm px-2.5 py-0.5 rounded-full uppercase tracking-tighter">
                                                  {topLabel}
                                                </span>
                                              </div>
                                            )}

                                            {/* Outer circle with backward arrow */}
                                            <div className="w-11 h-11 rounded-full bg-linear-to-b from-amber-400 to-amber-500 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform z-20 relative border-2 border-white">
                                              <div className="absolute inset-0 flex items-center justify-center">
                                                <div className="w-0 h-0 border-l-[7px] border-l-transparent border-r-[7px] border-r-transparent border-b-[10px] border-b-white transform rotate-0 mb-0.5"></div>
                                              </div>
                                            </div>

                                            {/* Label BELOW arrow */}
                                            {bottomLabel && (
                                              <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 whitespace-nowrap z-30 mt-2">
                                                <span className="text-[9px] font-black text-amber-100 bg-amber-600 border border-amber-500 shadow-sm px-2.5 py-0.5 rounded-full uppercase tracking-tighter mt-1">
                                                  {bottomLabel}
                                                </span>
                                              </div>
                                            )}
                                    ...
                                    <div className="w-1 h-8 bg-linear-to-b from-amber-300 to-amber-400 rounded-full"></div>
                                  </div>

                                  {/* Person Node Container */}
                                  <div className="relative group flex items-center">
                                    {/* Relationship Role (Left side of node) */}
                                    <div className="absolute right-[calc(100%+16px)] top-1/2 -translate-y-1/2 w-44 text-right animate-fadeIn z-20">
                                      <span className="text-[10px] font-black text-amber-700 bg-amber-50 px-2.5 py-1.5 rounded-lg border border-amber-200 shadow-sm uppercase tracking-tighter">
                                        {rel.cumulative_label}
                                      </span>
                                    </div>

                                    {/* Circle Node (Content only name) */}
                                    <div
                                      className={`w-22 h-22 rounded-full flex flex-col items-center justify-center p-2 z-10 transition-all duration-500 
                                      ${idx === filteredRelations.length - 1
                                          ? 'bg-linear-to-br from-green-500 to-green-600 border-2 border-green-400 shadow-xl scale-110 ring-4 ring-green-100'
                                          : 'bg-white border-2 border-amber-200 shadow-lg group-hover:border-amber-400 group-hover:scale-105'
                                        }`}
                                      title={rel.person?.name}
                                    >
                                      <User size={24} className={idx === filteredRelations.length - 1 ? 'text-white mb-0.5' : 'text-amber-600 mb-0.5'} />
                                      <div className={`text-[9px] font-black text-center leading-tight truncate w-16 uppercase ${idx === filteredRelations.length - 1 ? 'text-white' : 'text-amber-900'}`}>
                                        {rel.person?.name}
                                      </div>
                                    </div>
                  ...
                  <div className="h-24"></div> {/* Bottom spacing for better scroll experience */}
                </div>
              )}
              {/* PDF-Only Header */}
                          ...
                          <div className="relative mt-4 mb-4">
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <div className="relative my-2">
                                {(() => {
                                  // Check if in two-way mode with from/to phones
                                  const isInTwoWayMode = fromPhone.length === 10 && toPhone.length === 10;

                                  let topLabel = '';
                                  let bottomLabel = '';

                                  if (isInTwoWayMode) {
                                    // Force mother-son relationship for two-way mode
                                    topLabel = 'mother';
                                    bottomLabel = 'son';
                                  } else {
                                    // Use normal API arrow label
                                    const fullLabel = navigationHistory[index + 1]?.arrowLabel || 'Next';
                                    const labelParts = fullLabel.split(/->|→| - /);
                                    const hasSplit = labelParts.length > 1;
                                    topLabel = hasSplit ? labelParts[0].trim() : '';
                                    bottomLabel = hasSplit ? labelParts[1].trim() : fullLabel;
                                  }

                                  // Convert to Tamil labels
                                  const getTamilLabel = (label: string) => {
                                    if (label.toLowerCase().includes('younger brother')) return 'தம்பி';
                                    if (label.toLowerCase().includes('elder brother')) return 'அக்கா';
                                    if (label.toLowerCase().includes('younger sister')) return 'தங்கை';
                                    if (label.toLowerCase().includes('elder sister')) return 'அக்கா';
                                    if (label.toLowerCase().includes('mother')) return 'அம்மா';
                                    if (label.toLowerCase().includes('son')) return 'மகன்';
                                    if (label.toLowerCase().includes('daughter')) return 'மகள்';
                                    if (label.toLowerCase().includes('father')) return 'அப்பா';
                                    return label;
                                  };

                                  const topTamilLabel = getTamilLabel(topLabel);
                                  const bottomTamilLabel = getTamilLabel(bottomLabel);
                                  const hasSplit = topLabel && bottomLabel;

                                  return (
                                    <>
                                      {/* Label ABOVE arrow - mother */}
                                      {hasSplit && (
                                      ...
                                      <div className="w-10 h-10 rounded-full bg-linear-to-r from-blue-500 to-blue-600 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform z-10 relative">
                                        {/* Downward arrow */}
                                        <div className="absolute inset-0 flex items-center justify-center">
                                          <div className="w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[8px] border-b-white transform rotate-0"></div>
                                        </div>
                                      </div>

                                      {/* Label BELOW arrow - son */}
                                      <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 whitespace-nowrap z-20 mt-20">
                                        <span className="text-[9px] font-bold text-blue-700 bg-white/90 px-2 py-0.5 rounded-full shadow-xs border-blue-100 uppercase backdrop-blur-sm">
                                          {bottomTamilLabel}
                                        </span>
                                      </div>
                                    </>
    ...
    `}</style>
      {/* Relative Details Modal (Depth 1 of clicked person) */}
      <AnimatePresence>
        {showRelativesModal && (
          <div className="fixed inset-0 bg-amber-950/40 backdrop-blur-sm flex items-center justify-center z-200 p-4">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-hidden shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] border border-white/20 flex flex-col"
              onMouseDown={e => e.stopPropagation()}
            >
              {/* Header */}
              <div className="bg-linear-to-r from-amber-800 to-orange-700 px-6 py-4 text-white relative shrink-0">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black">{isTamil ? 'உறவினர் விவரங்கள்' : 'Relative Details'}</h3>
                      <p className="text-amber-100 text-xs font-medium tracking-wide uppercase opacity-80">
                        {isTamil ? 'நேரடி உறவுகள்' : 'Direct Connections'}
                      </p>
                    </div>
                  </div>
                  ...
                  {isTwoWayMode && (
                    <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto flex-1 max-w-xl">
                      {/* From Field */}
                      <div className="relative flex-1">
                        <Phone className="absolute left-2.5 top-1/2 transform -translate-y-1/2 text-amber-600/50" size={14} />
                        <input
                          type="tel"
                          value={fromPhone}
                          onChange={(e) => {
                          ...
                          placeholder={isTamil ? 'யாரிடமிருந்து...' : 'From...'}
                          className="w-full bg-white/90 border border-amber-500/30 rounded-lg py-1.5 pl-8 pr-3 text-amber-900 placeholder-amber-600/50 focus:outline-none focus:ring-2 focus:ring-amber-300 shadow-inner text-xs font-semibold"
                          maxLength={10}
                        />
                        {fromSuggestions.length > 0 && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-amber-200 rounded-xl shadow-xl z-50 overflow-hidden text-black">
                            <div className="max-h-40 overflow-y-auto">
                              {fromSuggestions.map((s) => (
                                <div
                                  ...
                                  className="p-2 hover:bg-amber-50 cursor-pointer border-b border-gray-100 last:border-0"
                                >
                                  <div className="text-xs font-bold">{s.full_name || s.label}</div>
                                  <div className="text-[10px] text-amber-600">{s.mobile_number || s.value}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          placeholder={isTamil ? 'யாருக்கு...' : 'To...'}
                          className="w-full bg-white/90 border border-amber-500/30 rounded-lg py-1.5 pl-8 pr-3 text-amber-900 placeholder-amber-600/50 focus:outline-none focus:ring-2 focus:ring-amber-300 shadow-inner text-xs font-semibold"
                          maxLength={10}
                        />
                        {toSuggestions.length > 0 && (
                      ...
                      <button
                        onClick={handleTwoWayConnect}
                        disabled={isConnecting}
                        className="bg-white/20 hover:bg-white/30 text-white rounded-lg px-3 py-1.5 text-xs font-bold shadow-sm flex items-center justify-center min-w-[36px]"
                        title={isTamil ? 'இணை' : 'Connect'}
                      >
                        {isConnecting ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <Link size={14} />}
                      </button>
                    </div>
                  )}

                  <button
                    onClick={() => setShowRelativesModal(false)}
                    className="text-white hover:text-orange-200 transition-all bg-white/10 rounded-xl p-2.5 hover:bg-white/20 hover:rotate-90 shadow-inner shrink-0"
                  >
                    <X size={20} />
                  </button>
                </div>
                <div className="absolute top-0 right-0 w-64 h-full bg-white/5 skew-x-12 transform translate-x-32 pointer-events-none"></div>
              </div>

              {/* Content Area */}
              <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-linear-to-b from-amber-50/30 to-white min-h-[300px]">
                {isFetchingRelatives ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-4">
                    <div className="w-12 h-12 border-4 border-amber-500 border-t-transparent rounded-full animate-spin" />
                    <p className="text-amber-600 font-bold uppercase tracking-widest animate-pulse">
                      {isTamil ? 'தேடுகிறது...' : 'Fetching Data...'}
                    </p>
                  </div>
                ) : !relativesModalData || !relativesModalData.connected_persons || relativesModalData.connected_persons.length === 0 ? (
                  <div className="text-center py-20">
                    <div className="w-24 h-24 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-6">
                      <Users className="text-amber-300" size={48} />
                    </div>
                    <p className="text-amber-900 font-bold text-xl uppercase tracking-widest">{isTamil ? 'உறவினர்கள் யாரும் இல்லை' : 'No Relatives Found'}</p>
                    <p className="text-amber-600 mt-2">{isTamil ? 'தகவல் எதுவும் கிடைக்கவில்லை' : 'Information not available'}</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {relativesModalData.connected_persons.map((item: any) => {
                      const person = item.person || item;
                      const label = item.relation_label?.label || item.relation_to_center;
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-black text-amber-900 text-sm truncate uppercase mb-0.5 group-hover:text-amber-600 transition-colors">
                              {person.full_name}
                            </div>
                          ...
                          {person.status === 'connected' && (
                            <div className="shrink-0 text-green-500 bg-green-50 p-1.5 rounded-lg shadow-inner">
                              <CheckCircle size={16} />
                            </div>
                          )}
                          {/* View Button */}
                          <button
                            onClick={() => {
                              console.log("View button clicked for person:", person);
                              // Can add more actions here if needed
                            }}
                            className="shrink-0 text-blue-600 bg-blue-50 p-1.5 rounded-lg shadow-inner hover:bg-blue-100 transition-colors"
                            title={isTamil ? 'பார்க்க' : 'View'}
                          >
                            <Eye size={16} />
                          </button>
                        </div>
              ...
              {/* Footer */}
              <div className="px-6 py-5 bg-amber-50/50 border-t border-amber-100 shrink-0">
                <div className="flex justify-center md:justify-end">
                  <button
                    onClick={() => setShowRelativesModal(false)}
                    className="w-full md:w-auto px-8 py-3 bg-white text-amber-900 border-2 border-amber-100 rounded-2xl hover:bg-amber-100 transition-all font-black uppercase tracking-widest text-xs active:scale-95 shadow-sm"
                  >
                    {isTamil ? 'மூடு' : 'Close'}
                  </button>
                </div>
