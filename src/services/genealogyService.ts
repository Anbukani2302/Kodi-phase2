...
class GenealogyService {
  async addRelative(data: AddRelativePayload): Promise<AddRelativeResponse> {
    ...
    console.log(
      "Endpoint:",
      "/api/genealogy/persons/{id}/add_relative_action/"
    );
    ...
    try {
      ...
      if (!action) {
        console.error(
          `No mapping found for relation: "${data.relation_to_me}" (normalized: "${relationKey}")`
        );
      ...
      if (error.response?.status === 403) {
        ...
        if (
          ...
          errorMessage.includes("cannot add relatives")
        ) {
          throw new Error(
            "This person has restricted family access. Only family members with proper permissions can add relatives."
          );
  ...
  async getRelationTypes(lang: string = "en"): Promise<RelationTypesResponse> {
    ...
    try {
      const response = await api.get(
        `/api/fixed-relations/dropdown_options/?lang=${lang}`
      );
  ...
  async getRelationsByLanguage(lang: string = "en"): Promise<any> {
    ...
    try {
      const response = await api.get(
        `/api/fixed-relations/by_language/?lang=${lang}`
      );
  ...
  async addRelativeAction(
    personId: number,
    data: AddRelativeActionPayload
  ): Promise<AddRelativeResponse> {
    console.log(
      `🔵🔵🔵 addRelativeAction CALLED for person ${personId}:`,
      data
    );
    ...
    try {
      ...
      if (error.response?.data?.code === "invalid_action") {
        console.log(
          ...
          error.response?.data?.valid_actions
        );
      }
  ...
  async getGenerationInfo(personId: number): Promise<GenerationInfo> {
    ...
    try {
      const response = await api.get(
        `/api/genealogy/person/${personId}/generation-info/`
      );
  ...
  async getAshramamRelations(personId: number): Promise<any> {
    ...
    try {
      const response = await api.get(
        `/api/genealogy/persons/${personId}/ashramam-relations/`
      );
  ...
  async addCustomRelative(
    ...
    console.log(
      ` API Call - addCustomRelative for person ${personId}:`,
      payload
    );
    try {
      const response = await api.post(
        `/api/genealogy/persons/${personId}/add-custom-relative/`,
        payload
      );
  ...
  async searchPersons(query: string): Promise<any> {
    ...
    try {
      const response = await api.get(
        `/api/genealogy/persons/search/?q=${query}`
      );
  ...
  async getRelationBetween(
    ...
    console.log(
      `📡 genealogyService.getRelationBetween: p1=${person1Id}, p2=${person2Id}, center=${centerPersonId}`
    );
    try {
  ...
  async getConnectedPersons(
    ...
    console.log(
      `API Call - getConnectedPersons for person ${personId} with max_depth ${maxDepth}`
    );
    try {
      const response = await api.get(
        `/api/genealogy/persons/${personId}/connected/?person_id=${personId}&max_depth=${maxDepth}`
      );
